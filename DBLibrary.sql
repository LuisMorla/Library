...
alter table `libros` ADD column `img_libro` varchar(300);

CREATE TABLE `img_autores` (
  `id_img` int auto_increment,
  `imagen_autor` varchar(300) NOT NULL,
  primary key(`id_img`),
  `id_autor` varchar(11),
  foreign key(`id_autor`) REFERENCES `autores`(`id_autor`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

INSERT INTO `img_autores` (`imagen_autor`, `id_autor`) VALUES ('https://frankwhiteauthor.com/static-assets/content/images/bio-images/fwheadshot-500px.jpg', '172-32-117');


Select * from `img_autores` as i inner join `autores` as a ON(i.`id_autor` = a.`id_autor`);
INSERT INTO `libros` (`nombre_libro`, `desc_libro`, `Precio_libro`, `id_autor`, `img_libro`) VALUES ('STORM COUNTRY POLLY', 'Esta es la descripción del libro', 56.25, '172-32-117', 'https://m.media-amazon.com/images/I/51bVIEElfTL._SY291_BO1,204,203,200_QL40_FMwebp_.jpg');
INSERT INTO `libros` (`nombre_libro`, `desc_libro`, `Precio_libro`, `id_autor`, `img_libro`) VALUES ('A History of Texas and Texans', 'Esta es la descripción del libro', 59.25, '172-32-117', 'https://m.media-amazon.com/images/I/41X4Ba7QyPL._SX331_BO1,204,203,200_.jpg');
